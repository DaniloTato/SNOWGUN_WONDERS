...
void forceHit(TangibleObject &tangible, int damage, float invincibilityTime,
  ...
  tangible.physics.setSpdx(knockSpeed.x, PhysicsComponent::SpeedType::MOVEMENT);

  tangible.physics.setSpdy(knockSpeed.y, PhysicsComponent::SpeedType::MOVEMENT);
...
void playerDamage(TangibleObject &tangible, const GeneralContext &ctx,
  ...
  auto &state = tangible.scripter.getState<PlayerDamageState>("playerDamage");
  ...
  Bullet *bulletHit = BulletManager::getInstance().isCollidingWithBullet(
      tangible, BulletManager::I_AM_PLAYER);

  if (bulletHit) {
    bulletHit->die();

    PlayerDamageFunctions::forceHit(
        tangible, 1, invincibilityDuration, 0.5f,
        {knockSpeed.x * -static_cast<float>(tangible.direction), knockSpeed.y});
    tangible.playSound("hit");
  }
  /*Check if player is colliding with bullet*/

  /*Check if player is colliding with enemy*/
  TangibleObject *enemyHit =
      EnemyManager::getInstance().isCollidingWithEnemy(tangible);
  ...
  if (enemyHit && !bulletHit) {
    PlayerDamageFunctions::forceHit(
        ...
        {knockSpeed.x * -static_cast<float>(tangible.direction), knockSpeed.y});
    tangible.playSound(
        GameState::getInstance().getPlayerHealth() > 0 ? "hit" : "die", 200);
  }
  /*Check if player is colliding with enemy*/
}
} // namespace script