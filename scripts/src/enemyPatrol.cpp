...
namespace script {
    ...
    void enemyPatrol(TangibleObject& tangible, const GeneralContext& ctx){
        ...
        if(!beingHurt){
            tangible.physics.setSpdx(1 * tangible.direction, PhysicsComponent::SpeedType::MOVEMENT);
        }
        tangible.physics.updateX(tangible.position);

        if (tangible.collider.horizontalLevelCollision(tangible.position) && !beingHurt) {
