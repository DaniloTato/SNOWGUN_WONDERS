...
UISlider::UISlider(const sf::Vector2f& position,
                   float width,
                   float minValue,
                   float maxValue,
                   float* boundValue,
...
void UISlider::handleEvent(const sf::Event& ev, sf::RenderWindow& window) {
    ...
    if (ev.type == sf::Event::MouseButtonPressed && ev.mouseButton.button == sf::Mouse::Left) {
        ...
        if (dx*dx + dy*dy <= (knob.getRadius() + 2.f)*(knob.getRadius() + 2.f)) {
            dragging = true;
            return;
        }
        ...
        if (trackRect.contains(static_cast<float>(mp.x), static_cast<float>(mp.y))) {
            float newPct = (mp.x - pos.x) / width;
