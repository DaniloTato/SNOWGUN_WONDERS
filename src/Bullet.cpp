...
Bullet::Bullet(RenderizerParameters params, const Animations &cachedAnimations,
               Bullet::Type t, const sf::Vector2f &initSpeed,
               const sf::Vector2f &accel, float dmgRadius, float range,
               bool shotByPlayer)
    : TangibleObject(params, cachedAnimations), type(t), acceleration(accel),
      damageRadius(dmgRadius), maxRange(range), spawnPos(params.position),
